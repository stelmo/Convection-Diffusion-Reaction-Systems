...
\label{eq_baltb1}
\end{equation}
If we assume adiabatic conditions (\ref{eq_baltb1}) simplifies to (\ref{eq_baltb2}) where $X$ is the conversion of the limiting species and $C_{pm}$ is the average heat capacity of the system.
\begin{equation}
...
In this problem Aris-Taylor analysis for laminar flow will be used to simplify the CDR equation. 
\begin{defn}
Aris-Taylor analysis is a procedure where the radial component of the CDR equation is lumped with the axial component resulting in a partial differential equation involving only one spatial variable. The lumped diffusion coefficient is defined as $D_a = D_{AB} + \frac{U^2R^2}{48D_{AB}}$.
\end{defn}
...
In this problem the full CDR problem for the scheme $A \rightarrow B$ is solved using the same first order kinetics as before. The full problem is shown in (\ref{eq_p2}).
\begin{equation}
\begin{aligned}
...
In this problem the isothermal assumption is dropped and the somewhat weaker adiabatic assumption replaces it. The model for the scheme $A + B \rightarrow 2C$ now becomes (\ref{eq_p4}). 
\begin{equation}
\begin{aligned}
...
In this problem a coupled isothermal CDR equation is used to model the reaction scheme $A+B \rightarrow 2C$. The model is shown in (\ref{eq_p7}). 
\begin{equation}
\begin{aligned}
&D_{e}(\frac{1}{r}\frac{\partial}{\partial r}(r\frac{\partial C_A}{\partial r}) + \frac{\partial^2 C_A}{\partial z^2}) - 2U(1-(\frac{r}{R})^2)\frac{\partial C_A}{\partial z} - kC_AC_B = \frac{\partial C_A}{\partial t}\\
&D_{e}(\frac{1}{r}\frac{\partial}{\partial r}(r\frac{\partial C_B}{\partial r}) + \frac{\partial^2 C_B}{\partial z^2}) - 2U(1-(\frac{r}{R})^2)\frac{\partial C_B}{\partial z} - kC_A C_B = \frac{\partial C_B}{\partial t}\\
&\text{Boundary Conditions:} \\
&\frac{\partial C_A}{\partial z}(z=L, \cdot, \cdot) = 0\\
&\frac{\partial C_A}{\partial r}(\cdot, r = R, \cdot) = 0 \\
&\frac{\partial C_A}{\partial r}(\cdot, r = 0, \cdot) = 0 \\
&C_A(z=0,\cdot, \cdot) = C_{A0} \\
&\frac{\partial C_B}{\partial z}(z=L, \cdot, \cdot) = 0\\
&\frac{\partial C_B}{\partial r}(\cdot, r = R, \cdot) = 0 \\
&\frac{\partial C_B}{\partial r}(\cdot, r = 0, \cdot) = 0 \\
&C_B(z=0,\cdot, \cdot) = C_{B0} \\
&\text{Initial Condition:} \\
& C_A(\cdot, \cdot, t= 0) = \beta(z)\\
& C_B(\cdot, \cdot, t= 0) = \alpha(z)
\end{aligned}
\label{eq_p7}
\end{equation}
%References
