...
& \text{with }k(C_A) = k_0 \exp(\frac{E}{R}(\frac{1}{T_0}-\frac{1}{T})) \\
& \text{and } T = T_0 + \frac{-H_{rxn}\frac{C_{A0}-C_A}{C_{A0}}}{C_{p_A}} \\
&\text{Boundary Conditions:} \\
&\frac{\partial C_A}{\partial z}(z=L, \cdot) = 0\\
&C_A(z=0, \cdot) = C_{A0} \\
&\text{Initial Condition:} \\
...
In this problem a coupled isothermal CDR equation is used to model the reaction scheme $A+B \rightarrow 2C$. The model is shown in (\ref{eq_p4}). 
\begin{equation}
\begin{aligned}
