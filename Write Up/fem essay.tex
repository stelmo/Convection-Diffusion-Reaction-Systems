...
\label{eq_p4d}
\end{equation}
Abusing our notation again and multiplying through by test functions $v$ and $z$ we have (\ref{eq_p4d1}) with $c_1 = \frac{UL}{D_a}$,  $c_{2A} = \frac{L^2C_{A0}}{D_a}$ and  $c_{2B} = \frac{L^2C_{B0}}{D_a}$.
\begin{equation}
\begin{aligned}
...
\label{eq_p4mat2}
\end{equation}
Solving this system of equations by Newton-Galerkin Method iteration proceeds in exactly the same was as in Section \ref{section_p3}. However, we are interested in the unsteady solution. We again rely on the Crank-Nicholson scheme to solve the unsteady problem. The Crank-Nicholson scheme is shown in (\ref{eq_p4mat3}) where we have discretised the time domain as usual.
\begin{equation}
\begin{aligned}
...
\label{eq_physp4}
\end{equation}
The model solution is shown in Figures \ref{fig_p4A} and \ref{fig_p4B}.
\begin{figure}[H] 
\centering
